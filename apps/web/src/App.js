...
export default function App() {
    ...
    if (!session) {
        return (_jsx("div", { className: "login-shell", children: _jsxs("section", { className: "login-card login-hero", children: [_jsxs("div", { className: "login-copy", children: [_jsxs("div", { className: "brand-lockup", children: [_jsx("img", { className: "brand-logo", src: "/Logoname.png", alt: "Sinergy Sol" }), _jsx("p", { className: "eyebrow", children: "Global Payroll Payouts" })] }), _jsxs("div", { className: "hero-copy", children: [_jsx("h1", { children: "LatAm payout operations console" }), _jsx("p", { className: "lede", children: "Run beneficiary onboarding, batch approvals, USDC funding, payout tracking, and exception handling from one place." })] }), _jsxs("div", { className: "login-form-shell", children: [_jsxs("label", { children: ["Email", _jsx("input", { value: loginForm.email, onChange: (event) => setLoginForm({ ...loginForm, email: event.target.value }) })] }), _jsxs("label", { children: ["Password", _jsx("input", { type: "password", value: loginForm.password, onChange: (event) => setLoginForm({ ...loginForm, password: event.target.value }) })] }), _jsx("button", { className: "primary", onClick: () => void handleLogin(), disabled: loading, children: loading ? "Signing in..." : "Enter workspace" }), _jsx("p", { className: "helper", children: "Demo users: finance, approver, compliance. Password: demo123" }), message ? _jsx("p", { className: "message", children: message }) : null] })] }), _jsx("aside", { className: "login-visual", "aria-hidden": "true", children: _jsx("div", { className: "banner-frame", children: _jsx("img", { className: "hero-banner", src: "/banner.png", alt: "" }) }) })] }) }));
    }
    return (_jsxs("div", { className: "app-shell", children: [_jsxs("aside", { className: "sidebar", children: [_jsxs("div", { children: [_jsx("div", { className: "sidebar-brand", children: _jsx("img", { className: "sidebar-logo", src: "/sinergysoldark.png", alt: "Sinergy Sol" }) }), _jsx("p", { className: "eyebrow", children: "Ops Core" }), _jsx("p", { className: "sidebar-copy", children: session.user.name }), _jsx("p", { className: "sidebar-copy role", children: session.user.role.replaceAll("_", " ") })] }), _jsx("nav", { children: navItems.map((item) => (_jsx(NavLink, { to: item.to, end: true, className: ({ isActive }) => (isActive ? "nav-link active" : "nav-link"), children: item.label }, item.to))) }), _jsx("button", { className: "ghost", onClick: () => {
                ...
                setSelectedBatch(null);
                                        ...
                                        const detail = await api.getBatch(session.accessToken, batchId);
                                        setSelectedBatch(detail);
                                    }, "Funding instructions generated."), onRefreshFunding: (instructionId) => runAction(async () => {
                                    ...
                                    }, "Funding status refreshed."), onRecordFallbackFunding: (instructionId, amount) => runAction(async () => {
                                    ...
                                    }, "Manual funding fallback recorded."), onDispatch: (payoutId) => runAction(async () => {
...
function BatchesPage(props) {
    const { data, selectedBatch, batchForm, setBatchForm, csvImport, setCsvImport, openBatch, onCreateBatch, onImportBatch, onQuote, onApprove, onGenerateFunding, onRefreshFunding, onRecordFallbackFunding, onDispatch, } = props;
